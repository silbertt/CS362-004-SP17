...
int playAdventurer(struct gameState * state)
{
  ...
  // Bug1: Unexpected increment of numBuys
  state->numBuys++;
  while(z-1>=0){
...
int playSmithy(struct gameState *state, int handPos)
{
  ...
  int i;
  //+3 Cards
  // Bug2: Unexpected number of cards drawn
  for (i = handPos; i < 3; i++)
  {
    drawCard(currentPlayer, state);
  }
...
int playSea_Hag(struct gameState *state)
{
  ...
  for (i = 0; i < state->numPlayers; i++){
    if (i != currentPlayer){
      ...
      //  Bug3: deckCount for player i is not decremented 
